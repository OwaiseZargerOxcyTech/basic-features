...
exports.register = [
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(201)
        .json({ message: "Verification OTP sent successfully", data: newUser });
    } catch (error) {
...
async function sendOtpMail(email, otp) {
  try {
    ...
    const mailOptions = {
      ...
      html: `<div>
        <p>Enter the otp to login in to your account.</p>
        <p>Your otp is: ${otp}</p>
      </div>`,
    };
...
exports.verifyLogin = async (req, res) => {
  ...
  try {
    const user = await prisma.user.findUnique({ where: { verifylogin: otp } });
